...
BEGIN
	...
	BEGIN TRY
		IF (@stock) > 0
			BEGIN
				UPDATE PRODUCTOS 
				SET CANTIDAD_STOCK = CANTIDAD_STOCK - CANTIDAD_PRODUCTOS
				FROM inserted
...
BEGIN
	IF @@ROWCOUNT = 1 
		BEGIN
			...
			UPDATE PEDIDOS 
			SET COSTO_TOTAL = COSTO_TOTAL + (COSTO_PRODUCTO * CANTIDAD_PRODUCTOS)
	...
	BEGIN
		...
		CLOSE CU_ACTUALIZA_MONTO_MATRICULAS
		DEALLOCATE CU_ACTUALIZA_MONTO_MATRICULAS;		
	END
END
GO
...
AS
	BEGIN
		...
		SET @codCliente = (SELECT COD_CLIENTE FROM CLIENTES WHERE ID_CLIENTE = @idCliente );
...
AS
	...
	BEGIN TRANSACTION
		BEGIN TRY
			...
			SET @msj = 'Producto agregado al carro';
	...
	BEGIN TRANSACTION
		BEGIN TRY
			...
			IF (NOT EXISTS (SELECT 1 FROM CLIENTES WHERE ID_CLIENTE = @idCliente))
				BEGIN
					INSERT INTO CLIENTES(NOMBRE,APELLIDO1,APELLIDO2,E_MAIL,TELEFONO,PROVINCIA,CANTON,DIRECCION)
					VALUES (@nombre,@apellido1,@apellido2,@email,@telefono,@provincia,@canton,@direccion);
